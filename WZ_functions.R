...
countReads <- function(F1scan, snps, minMapq = 20L, minPhred = 13L, nCont, saveForSimu = T, pool) {
  ...
  F1scan <- F1scan[abs(mapq) >= minMapq & phred >= minPhred, .(readPair, pos, base)]
...
pFgivenCountsSNPs <- function(c, r, group, ni = 10L, epsilon = 1 / 100, f = 0.5) {
  # this omputes the posterior probability of a value of f for individual SNPs, using snp
  # group information instead of c1, r1, c2, r2 (as r1 or r2 = 0 for a single
  # SNP)
  # c, r and group are integer vectors
  Fi <- 0:ni/(2*ni)
  Nominator <- vapply(Fi, function(Fi) {
    dbinom(c, size = r, prob = ifelse(
      group == 1L,
      yes = Fi,
      no = (1 - epsilon) * (0.5 - Fi) + epsilon
    ))
  }, numeric(length(c)))
...
pFgivenCountsHapl <- function(c1, r1, c2, r2, ni = 10L, epsilon = 0.01) {
...
assignToRegion <- function(bed, pos, olv = T) {
  ...
  bed[, id := 1:.N]
  bed <- bed[bed$scaffold %in% pos$scaffold, ]
  ...
  if (length(shorts) < nrow(dt)) {
    if (length(shorts) == 0) {
      ...
      # for each of these overlaping regions, we create a vector that represent
...
breakPoints <- function(gen, plot = F, symmetric = T, diag = T) {
  ...
  # we test if each non-heterogametic parent is heterozygous a both SNPs
  ...
  # for this, we use a trick. We make as if each SNP1 has recombined
  ...
  # we determine the initial start and end position of blocks
